...
$file_path = '.';
mkdir("$file_path/is-executable-variation2");

echo "\n*** Testing is_executable() on directory without execute permission ***\n";
chmod("$file_path/is-executable-variation2", 0444);
var_dump( is_executable("$file_path/is-executable-variation2") );  // exp: bool(false)
chmod("$file_path/is-executable-variation2", 0777);  // chmod to enable deletion of directory
...
create_files('.', 1, "numeric", 0755, 1, "w", $name_prefix, 4);
create_files('.', 1, "text", 0222, 1, "w", $name_prefix, 5);
create_files('.', 1, "numeric", 0711, 1, "w", $name_prefix, 6);
create_files('.', 1, "text", 0714, 1, "w", $name_prefix, 7);
create_files('.', 1, "numeric", 0744, 1, "w", $name_prefix, 8);
create_files('.', 1, "text", 0421, 1, "w", $name_prefix, 9);
create_files('.', 1, "text", 0712, 1, "w", $name_prefix, 10);
...
--CLEAN--
<?php
rmdir("./is-executable-variation2/");
?>
--EXPECTF--
