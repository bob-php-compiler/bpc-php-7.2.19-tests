...
if (substr(PHP_OS, 0, 3) != 'WIN') {
  ...
  $filename = "is_writable_root_check.tmp";
  $fp = fopen($filename, 'w');
  fclose($fp);
...
$file_path = '.';
mkdir("$file_path/is-writable-variation1");

// create a new temporary file
$fp = fopen("$file_path/is-writable-variation1/bar.tmp", "w");
fclose($fp);

/* array of files to be tested to check if they are writable
   using is_writable() function */
$files_arr = array(
  "$file_path/is-writable-variation1/bar.tmp",
  ...
  "$file_path/is-writable-variation1//bar.tmp",
  "$file_path//is-writable-variation1//bar.tmp",
  "$file_path/is-writable-variation1/*.tmp",
  "$file_path/is-writable-variation1/b*.tmp",

  /* Testing Binary safe */
  "$file_path/is-writable-variation1".chr(0)."bar.tmp",
  "$file_path".chr(0)."is-writable-variation1/bar.tmp",
  "$file_path".chr(0)."is-writable-variation1/bar.tmp",

  /* Testing directories */
  ".",  // current directory, exp: bool(true)
  "$file_path/is-writable-variation1"  // temp directory, exp: bool(true)
);
$counter = 1;
...
--CLEAN--
<?php
unlink("./is-writable-variation1/bar.tmp");
rmdir("./is-writable-variation1/");
?>
--EXPECTF--
