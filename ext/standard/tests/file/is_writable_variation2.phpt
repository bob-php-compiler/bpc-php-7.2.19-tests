...
$file_path = '.';
mkdir("$file_path/is-writable-variation2");

echo "\n*** Testing is_writable() on directory without write permission ***\n";
chmod("$file_path/is-writable-variation2", 0004);
var_dump( is_writable("$file_path/is-writable-variation2") );  // exp: bool(false)
var_dump( is_writeable("$file_path/is-writable-variation2") );  // exp: bool(false)
chmod("$file_path/is-writable-variation2", 0777);  // chmod to enable deletion of directory
...
create_files('.', 1, "text", 0114, 1, "w", $name_prefix, 7);
create_files('.', 1, "numeric", 0244, 1, "w", $name_prefix, 8);
