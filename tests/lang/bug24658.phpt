...
<?php
class foo {}
function no_typehint($a, $k = null) {
	var_dump($a);
}
function typehint(foo $a, $k = null) {
	var_dump($a);
}
function no_typehint_ref(&$a, $k = null) {
	var_dump($a);
}
function typehint_ref(foo &$a, $k = null) {
	var_dump($a);
}
$v = new foo();
$a = array(new foo(), 1, 2);
...
#1 %s(%d): array_walk(Array, 'typehint', unpassed)
#2 {main}
